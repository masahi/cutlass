...
__global__ void layernorm_twoPassAlgo_e8(float4* output,
                                         const float4* input,
                                         const float4* gamma,
                                         const float4* beta,
                                         const int m,
                                         const int n)
{
  ...
  for (int index = tid; index < n_8; index += bdimx) {
    ...
    local_sums[0] += (static_cast<float>(h1->x) - s_mean) * (static_cast<float>(h1->x) - s_mean);
    local_sums[0] += (static_cast<float>(h1->y) - s_mean) * (static_cast<float>(h1->y) - s_mean);
    local_sums[0] += (static_cast<float>(h2->x) - s_mean) * (static_cast<float>(h2->x) - s_mean);
    local_sums[0] += (static_cast<float>(h2->y) - s_mean) * (static_cast<float>(h2->y) - s_mean);
    local_sums[0] += (static_cast<float>(h3->x) - s_mean) * (static_cast<float>(h3->x) - s_mean);
    local_sums[0] += (static_cast<float>(h3->y) - s_mean) * (static_cast<float>(h3->y) - s_mean);
    local_sums[0] += (static_cast<float>(h4->x) - s_mean) * (static_cast<float>(h4->x) - s_mean);
    local_sums[0] += (static_cast<float>(h4->y) - s_mean) * (static_cast<float>(h4->y) - s_mean);
  }
  ...
  for (int index = tid; index < n_8; index += bdimx) {
    ...
    h1->x = half((static_cast<float>(l1->x) - s_mean)*s_variance*static_cast<float>(g1->x) + static_cast<float>(b1->x));
    h1->y = half((static_cast<float>(l1->y) - s_mean)*s_variance*static_cast<float>(g1->y) + static_cast<float>(b1->y));
    h2->x = half((static_cast<float>(l2->x) - s_mean)*s_variance*static_cast<float>(g2->x) + static_cast<float>(b2->x));
    h2->y = half((static_cast<float>(l2->y) - s_mean)*s_variance*static_cast<float>(g2->y) + static_cast<float>(b2->y));
    h3->x = half((static_cast<float>(l3->x) - s_mean)*s_variance*static_cast<float>(g3->x) + static_cast<float>(b3->x));
    h3->y = half((static_cast<float>(l3->y) - s_mean)*s_variance*static_cast<float>(g3->y) + static_cast<float>(b3->y));
    h4->x = half((static_cast<float>(l4->x) - s_mean)*s_variance*static_cast<float>(g4->x) + static_cast<float>(b4->x));
    h4->y = half((static_cast<float>(l4->y) - s_mean)*s_variance*static_cast<float>(g4->y) + static_cast<float>(b4->y));
...
void layernorm_half8(cutlass::MatrixCoord tensor_size,
  ...
  layernorm_twoPassAlgo_e8<<<grid, block, 0, stream>>>((float4 *)output,
						       (const float4 *)input,
						       (const float4 *)gamma,
						       (const float4 *)beta,
						       m, n);
...
void layernorm_half8_smem(cutlass::MatrixCoord tensor_size,
		     TensorRef<cutlass::half_t, layout::RowMajor> ref_output,
		     TensorRef<cutlass::half_t, layout::RowMajor> ref_input,
		     TensorRef<cutlass::half_t, layout::RowMajor> ref_gamma,
		     TensorRef<cutlass::half_t, layout::RowMajor> ref_beta,
  ...
  layernorm_twoPassAlgo_e8_smem<<<grid, block, smem_size_bytes, stream>>>((float4 *)output,
								    (const float4 *)input,
								    (const float4 *)gamma,
								    (const float4 *)beta,
								    m, n);
...
void layernorm_half8_smem_async(cutlass::MatrixCoord tensor_size,
		     TensorRef<cutlass::half_t, layout::RowMajor> ref_output,
		     TensorRef<cutlass::half_t, layout::RowMajor> ref_input,
		     TensorRef<cutlass::half_t, layout::RowMajor> ref_gamma,
		     TensorRef<cutlass::half_t, layout::RowMajor> ref_beta,
  ...
  layernorm_twoPassAlgo_e8_smem_async<<<grid, block, smem_size_bytes, stream>>>((float4 *)output,
								    (const float4 *)input,
								    (const float4 *)gamma,
								    (const float4 *)beta,
								    m, n);
